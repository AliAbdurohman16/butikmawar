...
class Product_model extends CI_Model {
    ...
    public function product_potential()
    {
        return $this->db->select('SUM(order_item.order_qty) as total, products.name as name')
                        ->from('order_item')->join('products', 'products.id = order_item.product_id')
                        ->group_by('name')->get()->result();
    }

    public function get_coupon()
    {
        return $this->db->select('COUNT(orders.coupon_id) as total, coupons.name as name')
                        ->from('orders')->join('coupons', 'coupons.id = orders.coupon_id')
                        ->group_by('name')->get()->result();
    }
}